#!/usr/bin/env bash
# make-toy.sh DEST — build a fresh toy repo with a two-task plan for the check-off eval.
set -euo pipefail
dest="$1"
rm -rf "$dest"
mkdir -p "$dest/docs/superpowers/plans" "$dest/src" "$dest/tests"
cd "$dest"
/usr/bin/git init -q -b main
/usr/bin/git config user.email eval@example.invalid
/usr/bin/git config user.name eval
printf '# greeter\n\nTwo tiny shell scripts, each with a test.\n' > README.md
printf '.superpowers/\n.worktrees/\n' > .gitignore
cat > docs/superpowers/plans/2026-09-11-greeter.md <<'PLAN'
...
PLAN
/usr/bin/git add -A
/usr/bin/git commit -q -m "chore: toy repo with greeter plan"
