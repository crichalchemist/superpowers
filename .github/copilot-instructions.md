...
**Core components:**
- **Skills** (`skills/*/SKILL.md`): YAML frontmatter + workflow instructions. Personal skills (`~/.claude/skills`) shadow plugin skills. Use `superpowers:` prefix to force plugin version.
- **Commands** (`commands/*.md`): User-facing slash commands (e.g., `/superpowers:brainstorm`, `/superpowers:interrogate`) that invoke skills.
...
- **Voice context**: Personal writing personality in `.github/writing-personality/` (gitignored) - Narrator agent references for authentic voice
...
- All narrative skills use the same `superpowers:` prefix (e.g., `superpowers:interrogate`)
...
2. `superpowers:story-plan` → Break into research/drafting tasks in `writing/plans/YYYY-MM-DD-<story>.md`
3. `superpowers:evidence-driven-drafting` → Dispatch specialist agents (Archivist, Analyst, Ethicist, Narrator, Skeptic, Community Listener)
4. `superpowers:fact-check-driven-revision` → CLAIM-CHECK-REVISE enforced (parallel to TDD for claims)
...
- **Narrator**: Storytelling, narrative structure, voice (references `.github/writing-personality/` for authentic voice if available)
...
- **Personal vs plugin skills** - personal skills shadow plugin by default, use `superpowers:` prefix to force plugin version
